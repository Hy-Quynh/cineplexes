...
module.exports = {
  getAll: async () => {
    return db.query(`SELECT "_movieID", "movieName", 'data:image/gif;base64,' || encode("moviePoster", 'base64') AS poster, "openingDay", "runningTime", "createdAt", trailer, genre FROM movies`,{ type: QueryTypes.SELECT });
  },
  ...
  detailMovie: async (movieID) => {
    return db.query(`SELECT "_movieID", "movieName", 'data:image/gif;base64,' || encode("moviePoster", 'base64') AS poster, to_char("openingDay", 'DD/MM/YYYY') as "openingDay", "runningTime", trailer, genre FROM movies WHERE "_movieID" = ${movieID}`,{
      type: QueryTypes.SELECT,
      plain: true
    });
  },
