(function ($) {
    ...
    $('#cineplexSelect').change(function (e) {
      ...
      if(value == '-1'){
        ...
        CreateToast('error', 'You not select', 'Please choose cinema complex');
        ...
        $.ajax({
            type: "GET",
            url: '/admin/cinema-by-cineplex',
            data: {
              cineplex_id: value,
            },
            success: function (data) {
              $('#cinemaSelect').find('option').remove().end();
              $('#cinemaSelect').append($("<option></option>").attr("value", '-1').text('Select'));
              $.each(data.cinemas_list, function(key, value) {
               $('#cinemaSelect').append($("<option></option>").attr("value", value._cinemaID).text(value.cinemaName));
              });
            },
            error: function (res) {
              if(res.responseJSON){
    ...
    $(document).on('submit', '#add-new-showtime', function (e) {
      e.preventDefault();
      ...
      const fare = $('#fare').val();
      const showAt = $('#showAt').val();
      if(cinemaId == '-1' || movieId == '-1' || startTime == '' || endTime == ''|| fare == '' || showAt == ''
      || !cinemaId || !movieId || !startTime || !endTime || !fare || !showAt){
        document.body.appendChild(div);
