(function ($) {
    ...
    $(document).on('submit', '#add-new-cinema', function (e) {
      e.preventDefault();
      const name = $('#name').val();
      const cineplexSelect = $('#cineplexSelect').val();
      const type = $('#type').val();
      const horizontal = $('#horizontal').val();
      const vertical = $('#vertical').val();
      if(name == '' || cineplexSelect == '-1' || type == '' || horizontal == ''|| vertical == ''
      || !name || !type || !horizontal || !vertical){
        ...
        $.ajax({
            type: "POST",
            url: '/admin/add-cinema',
            data: {
                name_cinema: name,
                cineplexId: cineplexSelect,
                type: type,
