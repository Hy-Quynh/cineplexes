(function ($) {
    ...
    let fromDate;
    let toDate;
    $('#fromDateMovie').change(function () {
       fromDate = $(this).val();
       $('.movie.active').removeClass('active');
    });
    $('#toDateMovie').change(function () {
      toDate = $(this).val();
      $('.movie.active').removeClass('active');
      if(fromDate && toDate){
        $.ajax({
          type: "POST",
          url: '/admin/movies-revenue',
          data: {
              start: fromDate,
              end: toDate
          },
          success: function (data) {
            if(data.status == 'success'){
              ...
              let totalPrice = [];
              let totalTickets = [];
              data.movies_revenue.forEach((value) => {
                moviesName.push(value.movieName);
                totalPrice.push(value.totalPrice);
                totalTickets.push(value.totalTickets);
              });
          ...
          error: function (res) {
            if(res.responseJSON){
              if(res.responseJSON.status == 'error'){
                ...
                CreateToast('error', 'Date selection error!', `${res.responseJSON.message}`);
              ...
              else if(res.responseJSON.status == 'empty'){
                document.body.appendChild(div);
                CreateToast('information', 'No data!', `${res.responseJSON.message}`);
                var thisToastSelect = toastCounter - 1;
                $(document).find("#toast_"+thisToastSelect).slideDown(600);
                setTimeout(function(){
                  $(document).find("#toast_"+thisToastSelect).slideUp(600,function(){
    ...
    jQuery(document).ready(function ($) {
      const from = $('#fromDateMovie').val();
      const to = $('#toDateMovie').val();
      if(from == '' && to == ''){
