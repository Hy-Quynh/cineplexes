        ...
        $.ajax({
          ...
          success: function (data) {
            if(data.status == 'success'){
              ...
              let totalPrice = [];
              let totalTickets = [];
              data.cineplexes_revenue.forEach((value) => {
                cineplexesName.push(value.cineplexName);
                totalPrice.push(value.totalPrice);
                totalTickets.push(value.totalTickets);
              });

              var ctx = document.getElementById('cineplexesChart').getContext('2d');
              var myChart = new Chart(ctx, {
                 type: 'bar',
                 data: {
                     labels: cineplexesName,
                     datasets: [
                      {
                       label: 'Total price',
                       data: totalPrice,
